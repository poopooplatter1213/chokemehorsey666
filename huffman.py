...
def huffman_tree(freq_dict):
    ...
    True
    >>> freq = {1: 5, 2: 9, 3: 12, 4: 13, 5: 16, 6: 45}
    >>> t = huffman_tree(freq)
    >>> t
    ...
    for item in freq_dict:
        newfreqtable.append((freq_dict[item], item))

    if len(newfreqtable) == 1:
        return HuffmanNode(newfreqtable[0][1], None, None)

    while len(newfreqtable) > 1:
        newfreqtable.sort()

        Firstitem = newfreqtable.pop(0)
        Firstfreq = Firstitem[0]
        Seconditem = newfreqtable.pop(0)
        Secondfreq = Seconditem[0]

        if type(Firstitem[1]) is str:
            Firstitem = HuffmanNode(Firstitem[1])
        else:
            Firstitem = Firstitem[1]

        if type(Seconditem[1]) is str:
            Seconditem = HuffmanNode(Seconditem[1])
        else:
            Seconditem = Seconditem[1]

        nHN = HuffmanNode(None, Firstitem, Seconditem)
        newfreqtable.append((Firstfreq + Secondfreq, nHN))

    return newfreqtable[0][1]
